package com.MCore.managers
{
	...
	public class PopUpManagerImpl
	{
		...
		public  function addPopUp(window : View, parent : Sprite, modal : Boolean = false, maskColor : uint = 0x000000, maskAlpha : Number = 0.5,tweener:Boolean = true) : void
		{
			...
			/*
			var tempScaleX:Number = window.scaleX;
			var tempScaleY:Number = window.scaleY;
			window.scaleX = window.scaleY = 0.8;*/
			if (tweener) 
			{	
				window.alpha = 0.3;
		...
		public  function removePopUp(window : View) : void
		{
			...
			if (o != null)
			{
				...
				/*if(o.modal)
				{
					TweenLite.to(o.modalMask,{alpha:0,time:0.6,onComplete:function():void{
		...
		private  function createModalWindow(parent : Sprite, o : PopUpData) : void
		{
			...
			if (parent is Stage)
			{
				rect = new Rectangle(0, 0, Stage(parent).stageWidth, Stage(parent).stageHeight);
			}
			else
			{
